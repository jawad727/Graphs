...
# Given: 
# - 500 Rooms in a graph
# - Each room CAN have 4 directions in which it can move, some have less (n, e, s, w)
# - You start in room 0 with n, e, s, w, 

# Task: Traverse through the entire maze and visit every room at least once

# Helpers:
# - player.currentRoom.id : returns id of current room
# - player.currentRoom.getExits() : returns array of directions you can move in
# - player.travel(direction) : moves you in that direction
# - player.currentRoom.getRoomInDirection(direction) : 

### ---

# Fill this out with all paths walked 
traversalPath = []
room_id = player.currentRoom.id # This is the starting room in the graph
direction_graph = {} # Create graph (which will contain all directions from each room as we fill them out)
direction_graph[room_id] = {i : '?' for i in player.currentRoom.getExits()} # create a question mark for every direction in the room
unexplored = None
...
while loops:
    ...
    for direction in current_room: # For every direction in current_room do the following
        try: # Using try because i keep getting keyErrors
            if current_room[direction] == '?': # If the directions we can move are '?' then move there and add the step to the traversalPath
                player.travel(direction)
                traversalPath.append(f'{direction}')
                if player.currentRoom.id in direction_graph: # if new room we just entered is in direction_graph, pass
                    pass
                else: 
                    # else create that room and place it in the direction_graph at the player.currentRoom.id and add all of its directions
                ...
                if direction == 'n': # If direction (away from my previous room) is 'n', do the following and then break out of the loop
                    direction_graph[player.currentRoom.id]['s'] = room_id # The current_room will set its opposite direction to the previous room (room_id)
                    room_id = player.currentRoom.id # room_id = the current room we are in (so we can reuse in the next iteration)
                    break
                elif direction == 'w':
                    ...
                    room_id = player.currentRoom.id
                    break
        except KeyError: # Prevents KeyErrors
            pass

        if '?' not in current_room.values(): # If there is nowhere to move from here (current room) do the following:
            for room in direction_graph: # iterate through every room we have logged into direction_graph
                if '?' in direction_graph[room].values(): # if "?" is a value in one of those rooms cahnge unexplored to True (because we need to keep searching)
                    unexplored = True

            # If unexplored is true, and we're at a dead end, it means we need to find the shortest path
            # Same BFS as ealier in the week, refer to projects/graph for the comments if forget how to use
            if unexplored == True: # (else loops will be False (see line 155 for code))
                visited = set()
                q = Queue()
                ...
                # At this point, path = an array of room IDs that will lead us back to the room with '?'s
                # For each room in the path we want to do the following:
                unexplored = False
                for room in path:
                    ...
                    if room in current_room.values():
                        ...
                        player.travel(direction) # Player travels in that direction
                        traversalPath.append(f'{direction}') # Add step to traversalPath
                        room_id = player.currentRoom.id # Room ID in now the current room
                        current_room = direction_graph[room_id] # Current room = the spot in the direction_graph where he room_id is a key
